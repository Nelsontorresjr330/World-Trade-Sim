...
def dfs(depth_bound, max_frontier, country, transformations, max_multiplier = 2):
    ...
    #Variables
    graph = {}  #Contains all the frontiers w/ all the nodes
...
def country_scheduler(your_country_name, resources_filename, initial_state_filename, output_schedule_filename, depth_bound, frontier_max_size, max_multiplier, output_list = []):
...
def main():
    ...
    #Initial Inputs
    depth_bound = input("\nMax Depth Bound (Must be greater than 1): ")
    while not depth_bound.isdigit():
       depth_bound = input("Max Depth Bound (Must be greater than 1): ")
    max_frontier_size = input("Max Frontier Size (Must be greater than 0): ")
    ...
    while not max_multiplier.isdigit():
       max_multiplier = input("Max Multiplier Value (Must be greater than 1): ")
    schedules = input("Number of Schedules (Must be greater than 0) (Schedules are created parallel to one another, will increase CPU demand): ")
    while not schedules.isdigit():
       schedules = input("Number of Schedules (Must be greater than 0) (Schedules are created parallel to one another, will increase CPU demand: ")
    ...
    for i in range(int(schedules)):
        ...
        p = multiprocessing.Process(target=country_scheduler,args=["Demacia", weights_path, initial_world_path, f"{output_path}_{i}.txt", int(depth_bound), int(max_frontier_size), int(max_multiplier), return_list])
        jobs.append(p)
        p.start()
    ...
    all_eus = []
    #Display best EUs for each schedule
    for iter, val in enumerate(return_list):
        all_eus.append(val[0])
